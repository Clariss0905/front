<template>
  <div>
    ...
    <div>
      ...
      >
        <el-table-column prop="id" label="申请编号" width="180" sortable>
        </el-table-column>
        <el-table-column prop="apply_id" label="申请人" width="180" sortable>
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="100">
          <template slot-scope="scope">
            <el-button
              @click="handleClickTrue(scope.row.id, scope.$index)"
              type="text"
              size="small"
              >批准</el-button
            >
            <el-button
              @click="handleClickFalse(scope.row.id, scope.$index)"
              type="text"
              size="small"
              >拒绝</el-button
            >
...
export default {
  ...
  methods: {
    getParams() {
      ...
      _this.uid = data;
      var url = "http://127.0.0.1:5000/searchApplyAll";
    ...
    submit() {
      ...
      axios({
        ...
        method: "POST",
        data: {
          student_id: _this.form.student_id,
    ...
    handleClickTrue(id, index) {
      this.applyData[index].state = "审核通过";
      this.changeState(id,1)
    },
    handleClickFalse(id, index) {
      this.applyData[index].state = "审核未通过";
      this.changeState(id,2)
    },
    changeState(id, state) {
        var path = "http://127.0.0.1:5000/updateState"
        axios({
        url: path,
        method: "POST",
        data: {
